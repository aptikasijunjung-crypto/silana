        ...
        <div class="card card-custom gutter-b">
            ...
            <div class="card-header border-0 py-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label font-weight-bolder text-dark">Surat Keterangan Usaha</span>
                    <span class="text-muted mt-3 font-weight-bold font-size-sm">{{ $data->kelurahan_name }}</span>
                </h3>
                <div class="card-toolbar">
                    <button type="button" class="btn btn-info font-weight-bolder font-size-sm modal-usaha" id="0"
            ...
            <div class="card-body py-0" id="tabel">
                ...
                @php
                    echo tabelKeteranganUsaha($tabel);
                @endphp
...
@section('jquery')
    ...
    <script>
        $('button.modal-usaha').click(function() {
            $('#lgModal').modal('show');
            id = $(this).attr('id');
            kelurahan_id = $(this).attr('kelurahan_id');
            csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            $.post("{{ route('keterangan.usaha.modal') }}", {
                id: id,
                _token: csrfToken,
